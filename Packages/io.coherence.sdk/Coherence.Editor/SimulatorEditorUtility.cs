...
namespace Coherence.Editor
{
    ...
    internal static class SimulatorEditorUtility
    {
        ...
        internal static EndpointData LastUsedEndpoint
        {
            get
            {
                if (!endpoint.ValidateLocalAddress().HasFlag(EndpointData.ValidationResult.ValidEndpoint))
                {
        ...
        internal static bool CanRunLocalSimulator(EndpointData endPoint) =>
            BuildPathHasSimulatorBuild() && ValidateIpAndPort(endPoint);

        private static bool ValidateIpAndPort(EndpointData endPoint) => ValidateIp(endPoint.host) &&
                                                                        endPoint.ValidateLocalAddress()
                                                                            .HasFlag(EndpointData.ValidationResult
                                                                                .ValidLocalEndpoint);
                    ...
                    if (CoherenceBridgeStore.TryGetBridge(SceneManager.GetActiveScene(), out var bridge))
                    {
                        if (bridge.Client != null)
                        {
                            bridge.Client.OnConnectedEndpoint +=
                                (connectedEndpoint) =>
                                {
                                    if (!connectedEndpoint.ValidateLocalAddress().HasFlag(EndpointData.ValidationResult.ValidLocalEndpoint))
                                    {
                                        return;
                                    }
